    ...
    function ajaxCategoryGenerate(filename, category_id = null) {
        $('.loading').show();
        $.ajax({
            type: 'GET',
            url: filename,
            success: function (data) {
                ...
                if(data.categories.length > 0){
                    var j = 0;
                    var catId = 0;
                    for(var i = data.categories[data.categories.length-1].id; i < (data.categories.length + data.categories[data.categories.length-1].id); i++){
                        catId = data.categories[j].id;
                        var dataCat = data.categories[i - data.categories[data.categories.length-1].id].name;
                        category += "<option value='"+ (catId) +"'>"+ dataCat +"</option>";
                        j++;
                    }
                    $('#addCategory').append(category);
                    $('#editCategory').append(category);
                }else{
    ...
    function ajaxCategoryGenerateForSearch(filename, category_id = null) {
        $('.loading').show();
        $.ajax({
            type: 'GET',
            url: filename,
